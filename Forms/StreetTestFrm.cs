...
{
    public partial class StreetTestFrm : Form
    {
        private static DataTable _dtTestAppointments;
        VisionTestfrm.enTestType testType = VisionTestfrm.enTestType.Street;
        int LDLAppId = -1;
        string Fullname = "";
        string Class = "", status ="", Nationalno="";
        ClsApplication Applicant;
        int TestTypeID = 3;
        public StreetTestFrm()
        {
            InitializeComponent();
        }
        public StreetTestFrm(int LDLAppID, ClsApplication applicant, string fullName, string Type, string Status, string NationNo)
        {
            InitializeComponent();
        ...
        private void StreetTestFrm_Load(object sender, EventArgs e)
        {
            _dtTestAppointments = ClsTestAppointments.GetAppointmentsWithLDLAppID(LDLAppId, TestTypeID);
            dgvAppointments.DataSource = _dtTestAppointments;
        ...
        private void editToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            bool edit = true;
            AddVisionTestAppointmentfrm scheduleTest = new AddVisionTestAppointmentfrm(testType, lblRecords.Text, LDLAppId, Fullname,
                Class,edit, Convert.ToInt16(dgvAppointments.CurrentRow.Cells[0].Value));
            scheduleTest.ShowDialog();
            _Refresh();
        }
